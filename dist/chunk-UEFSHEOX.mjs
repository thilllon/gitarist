...
import { existsSync, readFileSync, mkdirSync, writeFileSync, lstatSync, rmSync } from 'fs';
...
var branchPrefixes, workflowStatus, commitCategories, DEFAULT, Gitarist;
...
`;
      ...
      static async setup({ remote = DEFAULT.remote }) {
        ...
        writeFileSync(path.join(process.cwd(), ".env"), _Gitarist.getEnvTemplate({ owner, repo }), {
          encoding: "utf8",
          flag: "a+"
        });
      ...
      async simulateActiveUser({
        ...
        for (const key of Array(numberOfIssues).keys()) {
          ...
          await this.createCommitAndMakePullRequest({
            numberOfCommits: _.sample(_.range(minCommits, maxCommits + 1)),
            numberOfFiles: _.sample(_.range(minFiles, maxFiles + 1)),
            workingBranchPrefix,
            mainBranch,
            language
          });
    ...
    `;
      ...
      async createCommitAndMakePullRequest({
        ...
        const { data: issue } = await this.octokit.rest.issues.create({
          ...
          assignees: [_.sample(this.assigneeCandidates)]
        });
        console.debug(`create an issue #${issue.number}`);
        const label = _.sample(this.labelsCandidates);
        ...
        for (const commitCount of Array(numberOfCommits).keys()) {
          ...
          const { data: newCommit } = await this.octokit.rest.git.createCommit({
            owner: this.owner,
            repo: this.repo,
            message: `${_.sample(commitCategories)}: ${faker.lorem.sentences(1)}`,
            tree: newTree.sha,
            parents
          });
...
export { DEFAULT, Gitarist, __commonJS, __esm, branchPrefixes, commitCategories, init_github, workflowStatus };
//# sourceMappingURL=out.js.map
//# sourceMappingURL=chunk-UEFSHEOX.mjs.map